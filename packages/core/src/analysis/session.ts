...
import { writeSessionMarkdown } from "../../../workspace/src";
...
function getSessionEmail(recording: PostHogRecording): string | null {
...
export async function analyzeRenderedSession(params: {
  ...
  const markdownPath = await writeSessionMarkdown({
    ...
    frontmatter: {
      ...
      user_email: getSessionEmail(params.recording),
