          ...
          if (
            ...
            const attrs =
              snapshot.data.node.childNodes[1].childNodes[1].childNodes[0]
                .attributes;
            const parsedWidth = Number.parseInt(attrs.width, 10);
            const parsedHeight = Number.parseInt(attrs.height, 10);
...
export default async function constructEvents(
  ...
  if (sources[0].source === "blob_v2") {
    ...
    const keys = v2Sources
      .map((s) => Number.parseInt(String(s.blob_key), 10))
  ...
  const eventsUri = await uploadEventsToGCS({
    ...
    bucketName: bucket_name || process.env.VESAI_GCS_BUCKET || "ves.ai",
  });
