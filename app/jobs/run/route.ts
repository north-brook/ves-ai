    ...
    const projectResults = await Promise.all(
      Array.from(sourcesByProject.entries()).map(
        async ([projectId, projectSources]) => {
          ...
          const processedCount = await processPendingSessionsForProject(
            projectId,
            supabase,
          );
...
async function processPendingSessionsForProject(
  projectId: string,
  supabase: ReturnType<typeof adminSupabase>,
): Promise<number> {
  ...
  const { data: pendingSessions } = await supabase
    ...
    .eq("status", "pending")
    .order("session_at", { ascending: false })
    .limit(20); // Process up to 20 pending sessions per run
