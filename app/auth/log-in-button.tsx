  ...
  if (variant === "secondary") {
    if (authUser) {
      return (
        <div>
          <Link
            href="/home"
            className="border-border bg-background hover:bg-slate-50 dark:hover:bg-slate-900 hidden items-center gap-2 rounded-lg border px-6 py-2.5 font-medium transition-all duration-200 md:flex"
          >
            <span className="text-foreground font-semibold">Dashboard</span>
    ...
    return (
      <form action={googleAuthMutation.mutate}>
        <button
          type="submit"
          className="border-border bg-background hover:bg-slate-50 dark:bg-slate-900 hidden items-center gap-2 rounded-lg border px-6 py-2.5 font-medium transition-all duration-200 md:flex"
        >
          {googleAuthMutation.isIdle ? (
            <Google size={18} />
          ) : (
  ...
  return (
    <div
      className={cn(
        ...
        variant === "secondary" &&
          "bg-slate-50 dark:bg-slate-900 h-[46px] w-[223px] animate-pulse transition-all duration-200",
      )}
    />
  );
}
